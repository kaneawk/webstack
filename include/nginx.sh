...
Install_Nginx() {
  ...
  /bin/mv ${nginx_install_dir}/conf/nginx.conf{,_bk}
  if [[ "${Apache_version}" =~ ^[1-2]$ ]]; then
    /bin/cp ${oneinstack_dir}/config/nginx_apache.conf ${nginx_install_dir}/conf/nginx.conf
    ...
    /bin/cp ${oneinstack_dir}/config/nginx.conf ${nginx_install_dir}/conf/nginx.conf
