...
Install_Tengine() {
  ...
  ./configure --prefix=${tengine_install_dir} --user=${run_user} --group=${run_user} --with-http_stub_status_module --with-http_v2_module --with-http_ssl_module --with-http_gzip_static_module --with-http_realip_module --with-http_flv_module --with-http_mp4_module --with-http_concat_module=shared --with-http_sysguard_module=shared --with-openssl=../openssl-${openssl_version} --with-pcre=../pcre-${pcre_version} --with-pcre-jit ${malloc_module} ${nginx_modules_options}
  ...
  /bin/mv ${tengine_install_dir}/conf/nginx.conf{,_bk}
  if [[ "${Apache_version}" =~ ^[1-2]$ ]]; then
    /bin/cp ${oneinstack_dir}/config/nginx_apache.conf ${tengine_install_dir}/conf/nginx.conf
  elif [[ "${Tomcat_version}" =~ ^[1-2]$ ]] && [ ! -e "${php_install_dir}/bin/php" ]; then
    /bin/cp ${oneinstack_dir}/config/nginx_tomcat.conf ${tengine_install_dir}/conf/nginx.conf
  else
    /bin/cp ${oneinstack_dir}/config/nginx.conf ${tengine_install_dir}/conf/nginx.conf
