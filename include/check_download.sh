...
checkDownload(){
  ...
  if [ "${DB_yn}" == "y" ]; then
    ...
    if [ "${DB_version}" == "7" ]; then
      echo "Download percona 5.7..."
      src_url=${mirrorLink}/percona-server-${percona_5_7_version}.tar.gz && Download_src
    fi
    if [ "${DB_version}" == "8" ]; then
      echo "Download percona 5.6..."
